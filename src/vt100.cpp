/* -*- mode:c++ -*- */
/*************************************************************
  This sketch implements a simple vt100 terminal emulator
  based on vt100.c of avr-vt100 terminal emulator
  https://github.com/mkschreder/avr-vt100.git

  The copyright notice of the original file is duplicated here:
 *************************************************************/
/**
...
#define ili9340_fillRect M5.Lcd.fillRect
#define ili9340_setScrollMargins setupScrollArea
#define ili9340_setScrollStart scrollAddress

// Text font size
uint8_t textSize = VT100_CHAR_HEIGHT/8;
...
void setupScrollArea(uint16_t tfa, uint16_t bfa) {
  ...
  M5.Lcd.writedata((VT100_SCREEN_HEIGHT-tfa-bfa)>>8);  // Vertical Scrolling Area line count
  M5.Lcd.writedata(VT100_SCREEN_HEIGHT-tfa-bfa);
...
/* Imported from https://github.com/mkschreder/avr-vt100.git */
...
void _vt100_reset(void){
  ...
  term.char_height = 8;
  term.char_width = 6;
  term.back_color = TFT_BLACK;
  term.front_color = TFT_WHITE;
...
#ifdef M5STACKLCD
  M5.Lcd.setTextColor(term.front_color, term.back_color);
#else
  ili9340_setFrontColor(term.front_color);
  ili9340_setBackColor(term.back_color);
#endif
  ili9340_setScrollMargins(0, VT100_STATUS_AREA); 
...
void _vt100_clearLines(struct vt100 *t, uint16_t start_line, uint16_t end_line){
  for(int c = start_line; c <= end_line; c++){
    ...
    t->cursor_y = c; 
    ili9340_fillRect(0, VT100_CURSOR_Y(t), VT100_SCREEN_WIDTH, VT100_CHAR_HEIGHT, TFT_BLACK);
    t->cursor_y = cy;
  }
...
void _vt100_move(struct vt100 *term, int16_t right_left, int16_t bottom_top){
  ...
  if(new_x > VT100_WIDTH){
    ...
    term->cursor_x = VT100_WIDTH - ((new_x<0 ? -new_x : new_x) % VT100_WIDTH) + 1; 
  } else {
    term->cursor_x = new_x;
  }
...
#ifdef M5STACKLCD
  M5.Lcd.setTextColor(t->front_color, t->back_color);
  M5.Lcd.drawChar(ch, x, y, textSize);
#else
